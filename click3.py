...
# Init serial for paddle RE input
...
while running:
    ...
    # ボタンコントローラ（Pi pico）のボタンクリックチェック => MAIN_BTN_DOWN_EVENT または START_BTN_DOWN_EVENT を生成
    if ser.in_waiting > 0:
        line = ser.readline().decode('utf-8').rstrip()
        record = json.loads(line)
        print(f"Received: {record}")  # デバッグ用に受信したメッセージを表示
        if 'event' in record.keys() and record['event'] == 'button':
            if 'main_button' in record['state'].keys():
                main_button_down_event()
            elif 'start_button' in record['state'].keys():
                start_button_down_event()
